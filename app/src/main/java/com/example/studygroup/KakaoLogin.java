        ...
        LoginClient.getInstance().loginWithKakaoAccount(this, new Function2<OAuthToken, Throwable, Unit>() {
            ...
            public Unit invoke(OAuthToken oAuthToken, Throwable throwable) {
                if (oAuthToken != null) {//로그인 정보객체가 있다면
                    ...
                    UserApiClient.getInstance().me(new Function2<User, Throwable, Unit>() {
                        ...
                        public Unit invoke(User user, Throwable throwable) {
                            ...
                            if (user != null) {
                                ...
                                Glide.with(KakaoLogin.this).load(profileImageUrl).into(circleImageView);

                                //다음에 접속할때 로그인 다시 하지 않으려면 shardPreference에 로그인정보를 저장해두고 불러오도록 코드 추가..


                            } else {
                                Toast.makeText(KakaoLogin.this, "사용자 정보 요청 실패 : ", Toast.LENGTH_SHORT).show();
                            }
                    ...
                    Toast.makeText(KakaoLogin.this, "로그인 실패", Toast.LENGTH_SHORT).show();
                }
    ...
    public void clickLogout (View view){
        ...
        UserApiClient.getInstance().logout(new Function1<Throwable, Unit>() {
            ...
            public Unit invoke(Throwable throwable) {
                if (throwable != null)
                    Toast.makeText(KakaoLogin.this, "로그아웃 실패", Toast.LENGTH_SHORT).show();
                else {
                    Toast.makeText(KakaoLogin.this, "로그아웃", Toast.LENGTH_SHORT).show();

                    //로그인 회원정보 화면들 모두 초기화
                    tv_nickname.setText("닉네임");
                    tv_Email.setText("이메일");
                    Glide.with(KakaoLogin.this).load(R.mipmap.ic_launcher).into(circleImageView);
                }
                return null;
            }
        });
